...
uint8_t sd_ini(void)
{
	...
	SS_SD_DESELECT();
	for(i=0;i<100;i++) //80 импульсов (не менее 74) Даташит стр 91
	  SPI_Release();
	...
	temp = SD_cmd(CMD0, 0); 
	if(temp == 1) // Enter Idle state
	{
		//Вернёмся в нашу функцию инициализации и отправим команду CMD8 в нашу карту с целью узнать версию нашей карты. 
