...
#include "sdcard.h"
#include "main.h" // чтобы увидеть определениие пина включения СД карты

static void SDCARD_Select() {
    //HAL_GPIO_WritePin(SDCARD_CS_GPIO_Port, SDCARD_CS_Pin, GPIO_PIN_RESET); // было изначально
	HAL_GPIO_WritePin(SD_SPI1_OnOff_Pin_GPIO_Port, SD_SPI1_OnOff_Pin_Pin, GPIO_PIN_RESET); // определил свой пин включения карты
	HAL_Delay(20); // задержка от увака в кепке для устаканивания уровня напряжения
}

void SDCARD_Unselect() {
    //HAL_GPIO_WritePin(SDCARD_CS_GPIO_Port, SDCARD_CS_Pin, GPIO_PIN_SET);// было изначально
	HAL_GPIO_WritePin(SD_SPI1_OnOff_Pin_GPIO_Port, SD_SPI1_OnOff_Pin_Pin, GPIO_PIN_SET); // определил свой пин включения карты
	HAL_Delay(20); // задержка от увака в кепке для устаканивания уровня напряжения
}
...
static int SDCARD_WaitNotBusy() {
	...
	uint8_t R = 8;
    do {
	    R = SDCARD_ReadBytes(&busy, sizeof(busy));
	    busy = busy;
	    if(R < 0) {
            return -1;
        }
...
int SDCARD_Init() {
    ...
    */
	...
	for(int i = 0; i < 10; i++) { // 80 clock pulses
        an=HAL_SPI_Transmit(&SDCARD_SPI_PORT, &high, sizeof(high), HAL_MAX_DELAY);
